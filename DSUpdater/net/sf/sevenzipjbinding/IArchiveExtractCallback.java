...
public interface IArchiveExtractCallback extends IProgress {
	 ...
	 *            extract ask mode
	 * @return an instance of {@link ISequentialOutStream} sequential out stream
	 *         or <code>null</code> to skip the extraction of the current item
	 *         (with index <code>index</code>) and proceed with the next one
	 * 
	...
	/**
	 * Prepare operation. The index of the current archive item can be taken
	 * from the last call of {@link #getStream(int, ExtractAskMode)}.
	 * 
	 * @param extractAskMode
	 *            extract ask mode
	 * 
