 ...
 * <br>
 * The interface provides functionality to query archive and archive item
 * parameters and to extract archive items.<br>
 * <br>
 * The last call should be a call of the method
 * {@link ISevenZipInArchive#close()}. After this call no more methods should be
 * called.
 * 
...
public interface ISevenZipInArchive {
	/**
	 * Close archive. This method should be the last one called on the
	 * implementation of this interface. After this call no more methods should
	 * be called.
	 * 
	 * @throws SevenZipException
	 *             7-Zip or 7-Zip-JBinding intern error. Check exception message
	...
	/**
	 * Get value of property <code>propID</code> of the item with the index
	 * <code>index</code>.
	 * 
	 * @param index
	 ...
	 * Note: passing sorted <code>indices</code> array is more efficient. But it
	 * isn't suggested to manually sort indices with something like
	 * {@link Arrays#sort(int[])}. Sort indices only, if you can do it quicker,
	 * that generic sort algorithms: <code>O(n*log(n))</code>.
	 * 
	 * @param indices
	...
	/**
	 * Extract one item from archive. Multiple calls of this method are
	 * inefficient for some archive types.
	 * 
	 * @param index
